...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mci_review_submit'])) {
    ...
    if (!$isLoggedIn) {
        header('Location: /login.php?return=' . rawurlencode('/business.php?slug=' . $targetSlug . '#reviews'));
        exit;
    }
    ...
    if ($result['ok']) {
        header('Location: /business.php?slug=' . rawurlencode($targetSlug) . '&review_ok=1#reviews');
    } else {
        $err = $result['error'] ?? 'Something went wrong.';
        header('Location: /business.php?slug=' . rawurlencode($targetSlug) . '&review_err=' . rawurlencode($err) . '#reviews');
    }
    exit;
}
...
$businessPageUrl = $slug !== '' ? '/business.php?slug=' . rawurlencode($slug) : '/listings.php';
              ...
              <div class="d-inline-flex flex-wrap gap-2 align-middle">
                <?php foreach ($bizTags as $t): ?>
                  ...
                  <a
                    ...
                    href="/listings.php?tag=<?= rawurlencode($t) ?>"
          ...
          <div class="card-body">
            ...
            <?php if ($isLoggedIn): ?>
              <?php if ($userAlreadyReviewed): ?>
                ...
                <div class="mt-2">
                  <a class="small text-muted" href="/logout.php?return=<?= rawurlencode('/business.php?slug=' . $slug . '#reviews') ?>">Sign out</a>
                </div>
              <?php else: ?>
                <div class="mci-business-section-title mb-3">
                ...
                <form id="mciReviewForm" method="post" action="/business.php" class="mci-review-form">
            ...
            <?php else: ?>
              <div class="border rounded-3 p-4 text-center" style="background: linear-gradient(180deg, #f8fafc 0%, #fff 100%); border-color: var(--mci-border) !important;">
                ...
                <a class="btn btn-dark me-2" href="/login.php?return=<?= rawurlencode('/business.php?slug=' . $slug . '#reviews') ?>">Login</a>
                <a class="btn btn-outline-dark" href="/register.php?return=<?= rawurlencode('/business.php?slug=' . $slug . '#reviews') ?>">Register</a>
              </div>
            <?php endif; ?>
          </div>
        </div>
      ...
      <div class="col-12 col-lg-4 mci-business-sidebar-sticky">
        <div class="card mci-business-side-card border-0 bg-white mb-4">
          ...
          <div class="mci-business-side-actions d-flex flex-column gap-2">
            ...
            <?php endif; ?>
            <a class="btn btn-outline-dark w-100" href="/contact.php">Send enquiry</a>
          ...
          <div class="card-body">
            ...
            <div class="mci-nearby-radius btn-group flex-wrap mb-3" role="group" aria-label="Search radius">
              <?php foreach ($nearbyKmAllowed as $km): ?>
                ...
                $nearbyHref = '/business.php?' . http_build_query($params) . '#nearby';
                ?>
                <a
            ...
            <?php if (count($nearbyResults) === 0): ?>
              <p class="text-muted small mb-0">
                ...
                Try a larger radius or <a href="/listings.php">browse all</a>.
              </p>
            <?php else: ?>
      ...
      <div class="modal-body">
        ...
        <?php else: ?>
          <div class="alert alert-warning mb-0">
            ...
            <div class="mt-2">
              <a href="/login.php?return=<?= rawurlencode($businessPageUrl) ?>" class="btn btn-sm btn-dark me-2">Login</a>
              <a href="/register.php?return=<?= rawurlencode($businessPageUrl) ?>" class="btn btn-sm btn-outline-dark">Register</a>
            </div>
          </div>
        <?php endif; ?>
      </div>
